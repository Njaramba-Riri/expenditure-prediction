...
def profile(username):
    ...
    if user is None or (current_user.username != username and not current_user.can(Permission.ADMINISTER)):
        abort(404)
...
def update(username):
    ...
    form.username.data  = user.username or session.get('username')
    form.name.data      = user.full_name or session.get('name')
    form.location.data  = user.location or session.get('location')
    form.about_me.data  = user.about or session.get('about')
    return render_template('users/editProfile.html', form=form, user=user)
